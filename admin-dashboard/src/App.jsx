          ...
          if (msg.type === "DENSITY_UPDATE") {
            ...
            setGeoData((prev) => {
              ...
              newGeo.features = newGeo.features.map((f) => {
                ...
                if (randomUpdate)
                  f.properties.color = colors[randomUpdate.status] || "#22c55e";
                return f;
              });
              return newGeo;
            });
  ...
  const sendAlert = async () => {
    ...
    try {
      const response = await fetch(
        "https://flowscape.onrender.com/admin/broadcast?message=" +
          encodeURIComponent(alertMsg),
        { method: "POST" },
      );
      if (response.ok) {
    ...
    <div className="bg-background text-on-background h-screen overflow-hidden flex flex-col antialiased">
      <header className="fixed top-0 w-full z-50 bg-surface text-primary border-b border-surface-variant flex justify-between items-center px-container-padding h-16 shadow-sm">
        <div className="flex items-center gap-4">
          <span className="material-symbols-outlined text-primary cursor-pointer hover:bg-surface-container-low rounded-full p-2 transition-colors active:scale-95 duration-200">
            location_on
          </span>
        ...
        <div className="flex items-center gap-4">
          <span className="material-symbols-outlined text-on-surface-variant cursor-pointer hover:bg-surface-container-low rounded-full p-2 transition-colors active:scale-95 duration-200">
            notifications
          </span>
      ...
      <main className="flex-1 mt-16 flex overflow-hidden">
        <nav className="hidden md:flex flex-col w-64 bg-surface border-r border-surface-variant h-full p-container-padding shrink-0 overflow-y-auto">
          <div className="space-y-2">
            <a
              className="flex items-center gap-4 p-3 rounded-lg bg-secondary-container text-on-secondary-container transition-colors"
              href="#"
            >
            ...
            <a
              className="flex items-center gap-4 p-3 rounded-lg text-on-surface-variant hover:bg-surface-container-low transition-colors"
              href="#"
            >
          ...
          <div className="w-full lg:w-[400px] h-full bg-surface border-l border-surface-variant flex flex-col shrink-0 overflow-y-auto shadow-xl z-20">
            <div className="p-container-padding flex-1 space-y-gutter">
              <section className="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 shadow-sm">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <div className="relative w-24 h-24">
                    <svg
                      className="w-full h-full gauge-meter"
                      viewBox="0 0 100 100"
                    >
                      <circle
              ...
              <section className="bg-error-container/30 border border-error/20 rounded-xl p-4 shadow-sm relative overflow-hidden">
                ...
                <p className="font-body-sm text-body-md text-on-surface-variant mb-4 relative z-10">
                  Push alerts to mobile users in specific sectors or site-wide.
                </p>
                ...
                <div className="space-y-3 relative z-10">
                  ...
                  {broadcastStatus && (
                    <p
                      className={
                        "text-sm font-bold " +
                        (broadcastStatus.includes("Failed")
                          ? "text-red-600"
                          : "text-green-600")
                      }
                    >
              ...
              <section>
                <h3 className="font-label-lg text-label-lg text-on-surface-variant uppercase tracking-wider mb-3">
                  Sector Breakdown
                </h3>
                <div className="space-y-2">
                  <div className="bg-surface-container-lowest border border-outline-variant rounded-lg p-3 flex items-center justify-between hover:bg-surface-container-low transition-colors cursor-pointer">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="font-body-md text-body-md font-medium w-8 text-right">
                        92%
                      </span>
                    </div>
                  </div>
